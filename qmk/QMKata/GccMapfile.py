import re, io, os, glob as _glob


class GccMapfile:
    @staticmethod
    def find_map_file(firmware_path=None):
        """Auto-discover the most recently modified keychron .map file.

        Search order:
          1. <firmware_path>/.build/keychron*.map  (if firmware_path given)
          2. QMK_FIRMWARE env var /.build/keychron*.map
          3. Relative to this script: ../../.build/keychron*.map
          4. Current working directory: .build/keychron*.map
        Returns the path or raises FileNotFoundError.
        """
        candidates = []

        search_roots = []
        if firmware_path:
            search_roots.append(firmware_path)
        env_fw = os.environ.get("QMK_FIRMWARE")
        if env_fw:
            search_roots.append(env_fw)
        # relative to this script (QMKata lives inside the tools repo)
        script_dir = os.path.dirname(os.path.abspath(__file__))
        search_roots.append(os.path.join(script_dir, "..", ".."))
        # current working directory
        search_roots.append(os.getcwd())

        for root in search_roots:
            pattern = os.path.join(root, ".build", "keychron*.map")
            candidates.extend(_glob.glob(pattern))

        if not candidates:
            raise FileNotFoundError(
                "No keychron .map file found. Searched .build/ under: "
                + ", ".join(search_roots)
                + ". Pass --firmware-path or set QMK_FIRMWARE env var."
            )
    ...
    def __init__(self, map_file_path="", firmware_path=None):
        if not map_file_path:
            map_file_path = self.find_map_file(firmware_path)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Parse a GCC linker map file")
    parser.add_argument(
        "map_file",
        nargs="?",
        default="",
        help="Path to .map file (auto-discovers if omitted)",
    )
    parser.add_argument(
        "--firmware-path",
        default=None,
        help="Path to QMK firmware root for auto-discovery",
    )
    args = parser.parse_args()

    mapfile = GccMapfile(args.map_file, firmware_path=args.firmware_path)
    print(f"loaded: {mapfile.map_file_path}")
    print(
        "================================== functions =================================="
    )
    ...
    for fun in funs:
        if fun in mapfile.functions:
            print(
                f"{fun}: {mapfile.functions[fun]['section']} {hex(mapfile.functions[fun]['address'])},{mapfile.functions[fun]['size']} {mapfile.functions[fun]['object_file']}"
            )
        else:
            print(f"{fun}: not found")

    vars = ["keymaps", "g_rgb_matrix_host_buf", "dynld_func_buf"]
    for var in vars:
        if var in mapfile.variables:
            print(
                f"{var}: {mapfile.variables[var]['section']} {hex(mapfile.variables[var]['address'])},{mapfile.variables[var]['size']} {mapfile.variables[var]['object_file']}"
            )
        else:
